...
local function LOLI_Draw_LCD(event)
  ...
  local lcd_opt = 0
  if LCD_W == 480 then
    drawScreenTitle("Multi - LOLI RX outil de configuration")
    x_pos = 152
	x_inc = 90
    y_pos = 40
    y_inc = 20
  ...
  if loli_nok then
    if LCD_W == 480 then
      lcd.drawText(10,50,"The LOLI protocole n'est pas selectionne...", lcd_opt)
    else
      --Draw on LCD_W=128
      lcd.drawText(2,17,"LOLI protocole pas selectionner...",SMLSIZE)
    end
    return
  end
  ...
  if LCD_W == 480 then
	line = line + 1
	lcd.drawText(x_pos, y_pos+y_inc*line -2, "Voie", lcd_opt)
	lcd.drawText(x_pos+x_inc, y_pos+y_inc*line -2, "Fonction", lcd_opt)
  ...
  for i = 1, 8 do
    out = getValue("ch"..(i+8))
    lcd.drawText(x_pos, y_pos+y_inc*line, "VOIE"..i, lcd_opt)
