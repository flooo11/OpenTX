...
local function drawEngineMenu()
  lcd.clear()
  lcd.drawText(1, 0, "Votre modele a-t-il un moteur ?", 0)
  ...
  if engineMode == 1 then
    ...
    lcd.drawText(25, LCD_H-16, "VOIE choisie", 0);
...
-- Ailerons Menu
local aileronsModeItems = {"Non", "Oui", "Oui, 2 VOIES"}
local function drawAileronsMenu()
  lcd.clear()
  lcd.drawText(1, 0, "Votre modele a-t-il des ailerons ?", 0)
  lcd.drawFilledRectangle(0, 0, LCD_W, 8, GREY_DEFAULT+FILL_WHITE)
  ...
  if aileronsMode == 2 then
    ...
    lcd.drawText(20, LCD_H-16, "VOIES choisies", 0);
    lcd.drawText(LCD_W/2-19, LCD_H-8, ">>>", 0);
...
-- Flaps Menu
local flapsModeItems = {"Non", "Oui", "Oui, 2 VOIES"}
local function drawFlapsMenu()
  lcd.clear()
  lcd.drawText(1, 0, "Votre modele a-t-il des volets ?", 0)
  lcd.drawFilledRectangle(0, 0, LCD_W, 8, GREY_DEFAULT+FILL_WHITE)
...
-- Airbrakes Menu
local brakesModeItems = {"Non", "Oui", "Oui, 2 VOIES"}
local function drawBrakesMenu()
  lcd.clear()
  lcd.drawText(1, 0, "Votre modele a-t-il des aerofreins ?", 0)
  lcd.drawFilledRectangle(0, 0, LCD_W, 8, GREY_DEFAULT+FILL_WHITE)
...
-- Tail Menu
local tailModeItems = {"Prof(1)", "Prof(1) + Derive(1)", "Prof(2) + Derive(1)", "V-Tail(2)"}
local function drawTailMenu()
  lcd.clear()
  lcd.drawText(1, 0, "Config de la queue de votre modele", 0)
  lcd.drawFilledRectangle(0, 0, LCD_W, 8, GREY_DEFAULT+FILL_WHITE)
...
local function drawServoMenu(limits)
  ...
  lcd.drawText(25, 0, "servo min/max/neutre/direction ?", 0)
  lcd.drawFilledRectangle(0, 0, LCD_W, 8, GREY_DEFAULT+FILL_WHITE)
...
local function drawConfirmationMenu()
  ...
  local y = 12
  lcd.clear()
  lcd.drawText(48, 1, "Confirmez-vous ce choix ?", 0);
  lcd.drawFilledRectangle(0, 0, LCD_W, 9, 0)
  if engineMode == 1 then
    x, y = drawNextLine(x, y, "Moteur", thrCH1)
  end
  if aileronsMode == 1 then
    x, y = drawNextLine(x, y, "Ail", ailCH1)
  elseif aileronsMode == 2 then
    x, y = drawNextLine(x, y, "AilG", ailCH1)
    x, y = drawNextLine(x, y, "AilD", ailCH2)
  end
  if flapsMode == 1 then
    x, y = drawNextLine(x, y, "Volets", flapsCH1)
  elseif flapsMode == 2 then
    x, y = drawNextLine(x, y, "VoletG", flapsCH1)
    x, y = drawNextLine(x, y, "VoletD", flapsCH2)
  end
  if brakesMode == 1 then
    x, y = drawNextLine(x, y, "Aero", brakesCH1)
  elseif brakesMode == 2 then
    x, y = drawNextLine(x, y, "AeroG", brakesCH1)
    x, y = drawNextLine(x, y, "AeroD", brakesCH2)
  end
  if tailMode == 3 then
    x, y = drawNextLine(x, y, "V-TailG", eleCH1)
    x, y = drawNextLine(x, y, "V-TailD", eleCH2)
  else
    x, y = drawNextLine(x, y, "Derive", rudCH1)
    if tailMode == 1 then
      x, y = drawNextLine(x, y, "Prof", eleCH1)
    elseif tailMode == 2 then
      x, y = drawNextLine(x, y, "ProfG", eleCH1)
      x, y = drawNextLine(x, y, "ProfD", eleCH2)
    end
  end
  lcd.drawText(48, LCD_H-8, "[ENT] long pour confirmer", 0);
...
local function applySettings()
  ...
  elseif aileronsMode == 2 then
    addMix(ailCH1, MIXSRC_FIRST_INPUT+defaultChannel(3), "AilG", -100)
    addMix(ailCH2, MIXSRC_FIRST_INPUT+defaultChannel(3), "AilD")
  end
  if flapsMode == 1 then
    addMix(flapsCH1, MIXSRC_SA, "Volet")
  elseif flapsMode == 2 then
    addMix(flapsCH1, MIXSRC_SA, "VoletG")
    addMix(flapsCH2, MIXSRC_SA, "VoletD")
  end
  if brakesMode == 1 then
    addMix(brakesCH1, MIXSRC_SE, "Aero")
  elseif brakesMode == 2 then
    addMix(brakesCH1, MIXSRC_SE, "AeroG")
    addMix(brakesCH2, MIXSRC_SE, "AeroD")
  end
  if tailMode == 3 then
    addMix(eleCH1, MIXSRC_FIRST_INPUT+defaultChannel(1), "V-PrfG", 50)
    addMix(eleCH1, MIXSRC_FIRST_INPUT+defaultChannel(0), "V-DirG", -50, 1)
    addMix(eleCH2, MIXSRC_FIRST_INPUT+defaultChannel(1), "V-PrfD", 50)
    addMix(eleCH2, MIXSRC_FIRST_INPUT+defaultChannel(0), "V-DirD", 50, 1)
  else
    if tailMode > 0 then
      addMix(rudCH1, MIXSRC_FIRST_INPUT+defaultChannel(0), "Derive")
    end
    if tailMode == 1 then
      addMix(eleCH1, MIXSRC_FIRST_INPUT+defaultChannel(1), "Prof")
    elseif tailMode == 2 then
      addMix(eleCH1, MIXSRC_FIRST_INPUT+defaultChannel(1), "PrfG")
      addMix(eleCH2, MIXSRC_FIRST_INPUT+defaultChannel(1), "PrfD")
    end
  end
end
