...
-- Elevons Menu
local elevonsModeItems = {"2 VOIES"}
local function drawElevonsMenu()
  lcd.clear()
  lcd.drawText(1, 0, "Choisir les voies des ELEVONS", 0)
  lcd.drawFilledRectangle(0, 0, LCD_W, 8, GREY_DEFAULT+FILL_WHITE)
...
-- Rudder menu
local rudderModeItems = {"Non", "Oui"}

local function drawRudderMenu()
  lcd.clear()
  lcd.drawText(1, 0, "Votre modele a-t-il une derive ?", 0)
  lcd.drawFilledRectangle(0, 0, LCD_W, 8, GREY_DEFAULT+FILL_WHITE)
...
local function applySettings()
  ...
  addMix(elevCH1, MIXSRC_FIRST_INPUT+defaultChannel(1), "D-PrfG", 50)
  addMix(elevCH1, MIXSRC_FIRST_INPUT+defaultChannel(3), "D-AilG", 50, 1)
  addMix(elevCH2, MIXSRC_FIRST_INPUT+defaultChannel(1), "D-PrfD", 50)
  addMix(elevCH2, MIXSRC_FIRST_INPUT+defaultChannel(3), "D-AilD", -50, 1)
...
local function drawConfirmationMenu()
  ...
  x, y = drawNextLine(x, y, "Elevon G", elevCH1)
  x, y = drawNextLine(x, y, "Elevon D", elevCH2)
  if rudderMode == 1 then
    drawNextLine(x, y, "Derive", rudCH1)
  end
