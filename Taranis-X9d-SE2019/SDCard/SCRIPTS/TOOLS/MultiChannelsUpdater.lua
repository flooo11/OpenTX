...
local toolName = "TNS|Module Multi|TNE"
...
local function Multi_Draw_LCD(event)
  ...
  if module_conf["Type"] ~= 6 then
    if LCD_W == 480 then
      lcd.drawText(10,50,"Module Multi non configure...", BLINK)
    else
      --Draw on LCD_W=128
      lcd.drawText(2,17,"Module Multi non configure...",SMLSIZE)
    end
    return
  else
    lcd.drawText(x_pos, y_pos+y_inc*line,module_pos .. " Multi detecte.", lcd_opt)
    line = line + 1
  end

  --Channel order
  if (ch_order == -1) then
    lcd.drawText(x_pos, y_pos+y_inc*line,"Lecture ordre des voies impossible...", lcd_opt)
    line = line + 1
  end

  --Can't open file MultiChan.txt
  if file_ok == 0 then
    lcd.drawText(x_pos, y_pos+y_inc*line,"Fichier MultiChan.txt manquant...", lcd_opt)
    return
  end
  ...
  if protocol_name == "" or sub_protocol_name == "" then
    lcd.drawText(x_pos, y_pos+y_inc*line,"Protocole inconnu "..tostring(protocol).."/"..tostring(sub_protocol).." ...", lcd_opt)
    return
  elseif LCD_W > 128 then
    lcd.drawText(x_pos, y_pos+y_inc*line,"Protocole: " .. protocol_name .. " / Sous-Protocole: " .. sub_protocol_name, lcd_opt)
    line = line + 1
  else
    lcd.drawText(x_pos, y_pos+y_inc*line,"Protocole: " .. protocol_name, lcd_opt)
    line = line + 1
    lcd.drawText(x_pos, y_pos+y_inc*line,"Sous-Protocole: " .. sub_protocol_name, lcd_opt)
    line = line + 1
  end
  ...
  if LCD_W > 128 then
    lcd.drawText(x_pos, y_pos+y_inc*line,"Voies: " .. text1, lcd_opt)
    line = line + 1
    if text2 ~= "" then
  ...
  if event ~= EVT_VIRTUAL_ENTER and done == 0 then
    lcd.drawText(x_pos, y_pos+y_inc*line,"<ENT> Sauver", lcd_opt + INVERS + BLINK)
    return
  end
  
  lcd.drawText(x_pos, y_pos+y_inc*line,"Nom des voies.", lcd_opt)
  line = line + 1
  local output, nbr
  if done == 0 then
    for i,v in ipairs(channel_names) do
  ...
  lcd.drawText(x_pos, y_pos+y_inc*line,"Fait !", lcd_opt)
  line = line + 1
end
...
local function Multi_Init()
  ...
  local stick_names = { "Dir", "Prf", "Gaz", "Ail" }
